...
const checkAvailable = (field, val, err) => {
  if (!validator.isLength(val, { min: 1 })) { return err.push('Must have ' + field) }
}

const checkAlpha = (field, val, err) => {
  if (!validator.isAlpha(removeAccent(val), 'en-US', { ignore: ' ' })) { return err.push(field + ' can only contain alphabet') }
}

const checkAlphaNumeric = (field, val, err) => {
  if (!validator.isAlphanumeric(removeAccent(val), 'en-US', { ignore: ' ' })) { return err.push(field + ' can only contain alphabet, numbers') }
}
